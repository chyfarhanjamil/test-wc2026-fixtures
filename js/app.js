...
const App = (() => {
  ...
  function _buildTzSelector() {
    const wrap = document.getElementById('tzSelectorWrap');
    if (!wrap) return;
    ...
    select.onchange = () => {
      ...
      _afterTimezoneChange();
    };
  }

  function _afterTimezoneChange() {
    ...
    // Update tz badge in selector
    const badge = document.getElementById('tzBadge');
    if (badge) badge.textContent = tz.label;

    // Re-render everything that shows times
    TeamGrid.init(true); // reinit with fresh data
